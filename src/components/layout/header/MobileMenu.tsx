...
const MobileMenu: FC<MobileMenuProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setActiveMenuItem(null); // Reset submenu when main menu closes
    }
  ...
  const handleSubMenuClick = (e: React.MouseEvent, menuItem: any) => {
    if (menuItem.subSubMenu) {
      e.preventDefault();
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit="hidden"
        >
          {/* Drawer - Main Menu - Always visible when isOpen is true */}
          <motion.div
            ...
            className={`absolute right-0 top-0 h-screen w-full bg-white flex flex-col ${activeMenuItem ? 'z-40' : 'z-50'}`}
          >
            {/* Sticky Header */}
            <div className="sticky top-0 z-10 flex items-center justify-end bg-white px-8 py-4">
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-6 space-y-6">
              {mainNavigationLinks
                .filter((menu) => menu.title !== "Login")
                .map((menu, index) => (
                  <div key={index} className="space-y-4">
                    <H4 className="text-black font-medium!">{menu.title}</H4>
                    {menu.subMenu && (
                        ...
                        {menu.subMenu.map((item, idx) => (
                          <li key={idx}>
                            <Link
                              ...
                              className="group block rounded-lg px-2 py-1.5 transition hover:bg-gray-100"
                            >
          ...
          {/* SubMenu Panel - Overlays the drawer */}
          {activeMenuItem && (
            <MobileSubMenuPanel
              activeMenu={activeMenuItem.label}
              menuItems={activeMenuItem.subSubMenu}
              onBack={() => setActiveMenuItem(null)}
              onClose={onClose}
            />
          )}
        </motion.div>
